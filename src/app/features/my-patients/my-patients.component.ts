...
export class MyPatientsComponent implements OnInit {
    ...
    this.myPatientsService
      ...
      .subscribe(myPatients => {
        myPatients.forEach(elm => {
          this.myPatients.push(this.mappingMyPatients(elm.patient, elm.prohibited));
        });
  ...
  searchAction(event) {
    ...
    this.filtredPatients = (filterBy != null) ? this.performFilter(filterBy) : this.myPatients;
  }
  writeAction(item) {
    console.log(item);
  ...
  prohibitAction(item) {
    this.myPatientsService.prohibitePatient(item.users[0].id).subscribe(resp => {
      if (resp == true) {
          const index = this.filtredPatients.findIndex(elm => elm.users[0].id == item.users[0].id);
  ...
  deleteAction(item) {
    this.myPatientsService.deletePatientFromMyPatients(item.users[0].id).subscribe( resp => {
      this.filtredPatients = this.filtredPatients.filter(elm => elm.users[0].id != item.users[0].id);
    })
  }
  authorizeAction(item) {
    this.myPatientsService.authorizePatient(item.users[0].id).subscribe(resp => {
      if (resp == true) {
          const index = this.filtredPatients.findIndex(elm => elm.users[0].id == item.users[0].id);
